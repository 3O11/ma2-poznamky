...
\begin{proof}
	...
	\begin{itemize}
		...
		Potom existují $x \in X_1$ a $\varepsilon_0 > 0$ takové, že pro každé $\delta > 0$ existuje $x(\delta)$ takové, že
		$$d_1(x, x(\delta)) < \delta \qquad \text{ale} \qquad d_2(f(x), f(x(\delta))) \ge \varepsilon_0$$
		Položme $x_n = x(1/n)$. Potom $\lim_n x_n = x$ ale $(f(x_n))_n$ nemůže konvergovat k $f(x)$.
	\end{itemize}
\end{proof}
...
\begin{figure}[h]
	\centering
	\subfloat[\centering Obraz]{{\includegraphics[width=6cm]{01-image} }}%
	\hspace{4em}
	\subfloat[\centering Vzor]{{\includegraphics[width=6cm]{01-preimage} }}%
